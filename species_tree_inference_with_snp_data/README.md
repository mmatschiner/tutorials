...
* **vcftools:** Similar to bcftools, [vcftools](https://vcftools.github.io/downloads.html) ([Danecek et al. 2011](https://academic.oup.com/bioinformatics/article/27/15/2156/402296)) is a program for the manipulation of files in VCF format. vcftools is generally slower than bcftools and is increasingly being replaced by it. In this tutorial, vcftools is only used for a single command that could be replaced with an similar (but slightly biasing) bcftools command instead. The installation is thus not essential. Download files and installation instructions can be found at the [vcftools download webpage](https://vcftools.github.io/downloads.html).
...
* As we've seen above, some SNPs in file `NC_031969.f5.sub2.vcf.gz` were too close to each other on the chromosome to be considered independent markers. Some of these might have been removed in the last filtering step. Nevertheless, to ensure that no SNPs are closer to each other than a minimum distance of 100 bp, we can use a thinning function of vcftools or bcftools. The bcftools thinning method is in my view not ideal as it can lead to a biased set of SNPs (see this [request to the bcftools developers](https://github.com/samtools/bcftools/issues/1050)). To thin the dataset with vcftools, these commands can be used:
	...
	The similar but not exactly equivalent command with bcftools is
...
* The alignment file [`NC_031969.f5.sub4.nex`](res/NC_031969.f5.sub4.nex) and the file [`taxpartitions.txt`](res/taxpartitions.txt) that was written in the last step then need to be combined into a single file in Nexus format. Use the following command to do so:
